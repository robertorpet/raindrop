...
-spaces don't matter. Blank lines don't matter
-store values with <- vs. =
- use ctrl enter to run highlighted things in the script area, or the play button to run everything
```{r}
...
Review
prefer <- over =
cbind combine columns of equal length (there is also rbind for rows)
c concatenate
...
Review
$ means "through"
aggregate() is useful for summary stats "by" conditions
...
Loading your data
If your R file is in the same folder as your data, you're good. Otherwise you can use setwd() to pick the folder you want to load from
```{r}
data <- read.csv("week1.csv")

#installing and loading packages
library(readxl)
data <- read_excel("week1.xlsx", skip = 9,
...
Next week:
-best practices for raw data files
-visualization
